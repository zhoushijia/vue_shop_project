...
export default {
  ...
  methods: {
    ...
    // 保存 发送请求存储到后端
    async saveRole() {
      if (!this.selectRole) return this.$message.info('请选择角色')
      const { data: res } = await this.$http.put(`users/${this.userInfo.id}/role`, { id: this.userInfo.id, rid: this.selectRole })
